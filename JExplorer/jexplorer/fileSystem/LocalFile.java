...
import org.fseek.components.ExtensionDatabase;
...
public class LocalFile implements MyFile,Serializable
{
    ...
    public MyFile[] getFiles() throws IOException
    {
        if(!this.file.exists())
        {
            if(ComputerPanel.isDrive(this.file))
            {
                throw new IOException("Es befindet sich kein Datenträger im Laufwerk !\nOder der pfad ist nicht verfügbar !");
            }
            else
            {
                throw new IOException("Datei existiert nicht/nicht mehr !");
            }
        }
